...
class BankdebitConfirmModuleFrontController extends ModuleFrontController
{
    /**
    ...
    public function postProcess()
    {
        ...
        $cancelUrl = _PS_BASE_URL_ . __PS_BASE_URI__ . 'index.php?fc=module&module=bankdebit&controller=cancel&id_lang=' . $cart->id_lang . '&order_id=' . $this->module->currentOrder . '&token_order=' . md5(_COOKIE_KEY_ . 'orderId_' . $this->module->currentOrder) . '&key=' . $customer->secure_key;

        // Call PxOrder.Initialize8
        $params = array(
            'accountNumber' => '',
            'purchaseOperation' => $this->module->transactiontype, // for BankDebit mode uses SALE method only
            'price' => 0,
        ...
        $result = $this->module->getPx()->PrepareSaleDD2($params);
        if ($result['code'] !== 'OK' || $result['description'] !== 'OK' || $result['errorCode'] !== 'OK') {
            $order->setCurrentState(Configuration::get('PS_OS_ERROR'));
            die(Tools::displayError($this->module->getVerboseErrorMessage($result)));
        }
