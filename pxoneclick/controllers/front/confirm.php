 ...
 * @since 1.5.0
 */
class PxoneclickConfirmModuleFrontController extends ModuleFrontController
{
    ...
    public function postProcess()
    {
        ...
        if (count($agreement) > 0) {
            ...
            $agreement_status = self::AGREEMENT_NOTEXISTS;
        }

        // Use Direct AutoPay method
        if ($agreement_status === self::AGREEMENT_VERIFIED) {
            ...
            $result = $this->module->getPx()->AutoPay3($params);
            if ($result['errorCodeSimple'] !== 'OK') {
                ...
                $other_error_codes = array(
                    'PaymentRefusedByFinancialInstitution', 'CreditCard_Error', 'REJECTED_BY_ACQUIRER', 'Error_Generic',
                    '3DSecureDirectoryServerError', 'AcquirerComunicationError', 'AmountNotEqualOrderLinesTotal', 'CardNotEligible'
                );
                if (in_array($result['errorCode'], $other_error_codes)) {
                    ...
                    $result = $this->module->getPx()->DeleteAgreement($params);
                    if ($result['code'] !== 'OK' || $result['description'] !== 'OK' || $result['errorCode'] !== 'OK') {
                        $order->setCurrentState(Configuration::get('PS_OS_ERROR'));
                        die(Tools::displayError($this->module->getVerboseErrorMessage($result)));
                    }

                    // Remove Agreement From Database
                    $agreement_status = self::AGREEMENT_DELETED;
                } else {
                    $order->setCurrentState(Configuration::get('PS_OS_ERROR'));
                    die(Tools::displayError($this->module->getVerboseErrorMessage($result)));
                }
                ...
                // Redirect to success
                $redirectUrl = _PS_BASE_URL_ . __PS_BASE_URI__ . 'order-confirmation.php?key=' . $customer->secure_key . '&id_cart=' . (int)$this->context->cart->id . '&id_module=' . (int)$this->module->id . '&id_order=' . (int)$this->module->currentOrder . '&transaction_id=' . $result['transactionNumber'] . '&autopay_token=' . md5(_COOKIE_KEY_ . 'transactionId_' . $result['transactionNumber']);
                Tools::redirect($redirectUrl);
                return;
            }
        }

        // Use Redirect Method to First use
        // Remove Deleted Agreement
        if ($agreement_status === self::AGREEMENT_DELETED) {
            Db::getInstance()->Execute('DELETE FROM ' . _DB_PREFIX_ . 'payex_autopay WHERE customer_id = ' . (int)$customer_id . ';');
        }

        // Create Agreement Ref
        if ($agreement_status === self::AGREEMENT_NOTEXISTS || $agreement_status === self::AGREEMENT_DELETED) {
            // Call PxAgreement.CreateAgreement3
            $params = array(
                'accountNumber' => '',
                'merchantRef' => $this->module->agreement_url,
                'description' => $this->module->displayName,
                'purchaseOperation' => $this->module->transactiontype,
                'maxAmount' => round($this->module->max_amount * 100),
                'notifyUrl' => '',
                'startDate' => '',
                'stopDate' => ''
            );
            ...
            $this->module->getPx()->setEnvironment($this->module->accountnumber, $this->module->encryptionkey, (bool)$this->module->mode);
            $result = $this->module->getPx()->CreateAgreement3($params);
            if ($result['code'] !== 'OK' || $result['description'] !== 'OK' || $result['errorCode'] !== 'OK') {
                $order->setCurrentState(Configuration::get('PS_OS_ERROR'));
                die(Tools::displayError($this->module->getVerboseErrorMessage($result)));
            }

            Db::getInstance()->Execute('INSERT INTO ' . _DB_PREFIX_ . 'payex_autopay (customer_id, agreement_ref, max_amount) VALUES(' . (int)$customer_id . ', "' . pSQL($result['agreementRef']) . '", ' . (float)$this->module->max_amount . ');');

            $agreement = array(
                'customer_id' => $customer_id,
                'agreement_ref' => $result['agreementRef'],
                'max_amount' => $this->module->max_amount
            );
        }

        $returnUrl = _PS_BASE_URL_ . __PS_BASE_URI__ . 'order-confirmation.php?key=' . $customer->secure_key . '&id_cart=' . (int)$this->context->cart->id . '&id_module=' . (int)$this->module->id . '&id_order=' . (int)$this->module->currentOrder;
