...
{else}
    {if $error}
        <p class="warning">{l s='Error:' mod='factoring'} {$error}</p>
    {/if}
    <h3>{l s='PayEx Factoring' mod='factoring'}</h3>
    <form action="{$link->getModuleLink('factoring', 'confirm', [], true)|escape:'html'}" method="post">
        <p>
            <img src="{$this_path_px}logo.gif" alt="{l s='PayEx' mod='factoring'}" width="86" height="49" style="float:left; margin: 0px 10px 5px 0px;" />
        ...
        <p>
            ...
            {else}
                {l s='We allow the following currency to be sent via PayEx:' mod='factoring'}&nbsp;<b>{$currencies.0.name}</b>
        ...
        <p class="cart_navigation" id="cart_navigation">
            ...
            <a href="{$link->getPageLink('order', true, NULL, "step=3")|escape:'html'}" class="button_large">{l s='Other payment methods' mod='factoring'}</a>
        </p>
    </form>
{/if}
