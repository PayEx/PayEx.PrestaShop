...
class Bankdebit extends PaymentModule
{
    private $_html = '';
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('Payex Bank Debit');
        $this->description = $this->l('Accept payments for your products via PayEx Bank Debit.');
    ...
    public function install()
    {
        ...
        Db::getInstance()->execute("CREATE TABLE IF NOT EXISTS `" . _DB_PREFIX_ . "payex_bankdebit_transactions` (
        ...
        Configuration::updateValue('PX_BD_ACCOUNT_NUMBER', '');
        Configuration::updateValue('PX_BD_ENCRYPTION_KEY', '');
    ...
    private function _postProcess()
    {
        if (Tools::isSubmit('btnSubmit')) {
            ...
            Configuration::updateValue('PX_BD_ACCOUNT_NUMBER', Tools::getValue('accountnumber'));
            Configuration::updateValue('PX_BD_ENCRYPTION_KEY', Tools::getValue('encryptionkey'));
        ...
        $this->_html .=
			...
			<legend><img src="../img/admin/contact.gif" />' . $this->l('Settings') . '</legend>
				<table border="0" width="500" cellpadding="0" cellspacing="0" id="form">
					...
					<tr>
						<td width="130" style="vertical-align: top;">' . $this->l('Banks') . '</td>
						<td>
                            <select id="banks" name="banks[]" class=" select multiselect" size="10" multiple="multiple">
                                <option value="NB" ' . (in_array('NB', Tools::getValue('banks', $this->banks)) ? 'selected="selected"' : '') . '>Nordea Bank</option>
                                <option value="FSPA" ' . (in_array('FSPA', Tools::getValue('banks', $this->banks)) ? 'selected="selected"' : '') . '>Swedbank</option>
                                <option value="SEB" ' . (in_array('SEB', Tools::getValue('banks', $this->banks)) ? 'selected="selected"' : '') . '>Svenska Enskilda Bank</option>
                                <option value="SHB" ' . (in_array('SHB', Tools::getValue('banks', $this->banks)) ? 'selected="selected"' : '') . '>Handelsbanken</option>
                                <option value="NB:DK" ' . (in_array('NB:DK', Tools::getValue('banks', $this->banks)) ? 'selected="selected"' : '') . '>Nordea Bank DK</option>
                                <option value="DDB" ' . (in_array('DDB', Tools::getValue('banks', $this->banks)) ? 'selected="selected"' : '') . '>Den Danske Bank</option>
                                <option value="BAX" ' . (in_array('BAX', Tools::getValue('banks', $this->banks)) ? 'selected="selected"' : '') . '>BankAxess</option>
                                <option value="SAMPO" ' . (in_array('SAMPO', Tools::getValue('banks', $this->banks)) ? 'selected="selected"' : '') . '>Sampo</option>
                                <option value="AKTIA" ' . (in_array('AKTIA', Tools::getValue('banks', $this->banks)) ? 'selected="selected"' : '') . '>Aktia, Säästöpankki</option>
                                <option value="OP" ' . (in_array('OP', Tools::getValue('banks', $this->banks)) ? 'selected="selected"' : '') . '>Osuuspanki, Pohjola, Oko</option>
                                <option value="NB:FI" ' . (in_array('NB:FI', Tools::getValue('banks', $this->banks)) ? 'selected="selected"' : '') . '>Nordea Bank Finland</option>
                                <option value="SHB:FI" ' . (in_array('SHB:FI', Tools::getValue('banks', $this->banks)) ? 'selected="selected"' : '') . '>SHB:FI</option>
                                <option value="SPANKKI" ' . (in_array('SPANKKI', Tools::getValue('banks', $this->banks)) ? 'selected="selected"' : '') . '>SPANKKI</option>
                                <option value="TAPIOLA" ' . (in_array('TAPIOLA', Tools::getValue('banks', $this->banks)) ? 'selected="selected"' : '') . '>TAPIOLA</option>
                                <option value="AALAND" ' . (in_array('AALAND', Tools::getValue('banks', $this->banks)) ? 'selected="selected"' : '') . '>Ålandsbanken</option>
                            </select>
						</td>
					</tr>
					<tr>
    ...
    public function hookPayment($params)
    {
        ...
        foreach($this->banks as $bank_code) {
    ...
    public function hookBackOfficeHeader()
    {
        ...
        if (Tools::isSubmit('process_fetch') && Tools::getValue('payex_transaction_id')) {
            ...
            if (!empty($result['transactionStatus'])) {
                if (!Db::getInstance()->Execute('UPDATE `' . _DB_PREFIX_ . 'payex_bankdebit_transactions` SET transaction_status=' . pSQL((int)$result['transactionStatus']) . ', transaction_data="' . pSQL(serialize($result)) . '" WHERE transaction_id=' . $payex_transaction_id . ';')) {
        ...
        if (Tools::isSubmit('process_capture') && Tools::getValue('payex_order_id') && Tools::getValue('payex_transaction_id')) {
            ...
            if ($result['code'] !== 'OK' || $result['description'] !== 'OK' || $result['errorCode'] !== 'OK') {
                $output = array(
                    'status' => 'error',
                    'message' => $this->l($result['errorCode'] . ' (' . $result['description'] . ')')
                );
                die(Tools::jsonEncode($output));
            }
    ...
    public function hookAdminOrder($params)
    {
        ...
        if ($order->module === $this->name) {
            ...
            $transactions = Db::getInstance()->executeS('SELECT * FROM ' . _DB_PREFIX_ . 'payex_bankdebit_transactions WHERE order_id = ' . $order_id . ' ORDER BY date DESC;');
    ...
    public function addTransaction($order_id, $transaction_id, $transaction_status, $transaction_data, $date = null)
    {
        ...
        // Save Transaction
        if (!Db::getInstance()->Execute(sprintf('INSERT INTO `' . _DB_PREFIX_ . 'payex_bankdebit_transactions` (order_id, transaction_id, transaction_status, transaction_data, date) VALUES (%s, %s, %s, "%s", "%s");',
    ...
    {
        $transaction = Db::getInstance()->executeS('SELECT * FROM ' . _DB_PREFIX_ . 'payex_bankdebit_transactions WHERE transaction_id = ' . (int) $transaction_id . ';');
