{if $status == 'ok'}
    <p>{l s='Your order on %s is complete.' sprintf=$shop_name mod='bankdebit'}
        ...
        <br /><br />{l s='For any questions or for further information, please contact our' mod='bankdebit'} <a href="{$link->getPageLink('contact', true)|escape:'html'}">{l s='customer support' mod='bankdebit'}</a>.
    </p>
{else}
    {if $status == 'pending'}
    ...
    {else}
        <p class="warning">
            ...
            <a href="{$link->getPageLink('contact', true)|escape:'html'}">{l s='customer support' mod='bankdebit'}</a>.
            <br />
