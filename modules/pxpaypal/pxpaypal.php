...
class Pxpaypal extends PaymentModule
{
    private $_html = '';
    ...
    public function __construct()
    {
        ...
        // Init Configuration
        $config = Configuration::getMultiple(array('PAYPAL_PX_ACCOUNT_NUMBER', 'PAYPAL_PX_ENCRYPTION_KEY', 'PAYPAL_PX_TESTMODE', 'PAYPAL_PX_TRANSACTION_TYPE', 'PAYPAL_PX_RESPONSIVE'));
        $this->accountnumber = isset($config['PAYPAL_PX_ACCOUNT_NUMBER']) ? $config['PAYPAL_PX_ACCOUNT_NUMBER'] : '';
        $this->encryptionkey = isset($config['PAYPAL_PX_ENCRYPTION_KEY']) ? $config['PAYPAL_PX_ENCRYPTION_KEY'] : '';
        $this->mode = isset($config['PAYPAL_PX_TESTMODE']) ? $config['PAYPAL_PX_TESTMODE'] : 1;
        $this->transactiontype = isset($config['PAYPAL_PX_TRANSACTION_TYPE']) ? $config['PAYPAL_PX_TRANSACTION_TYPE'] : 'AUTHORIZATION';
        $this->paymentview = 'PAYPAL';
        $this->responsive = isset($config['PAYPAL_PX_RESPONSIVE']) ? (bool)$config['PAYPAL_PX_RESPONSIVE'] : false;
        ...
        $this->displayName = $this->l('PayPal via PayEx');
    ...
    public function install()
    {
        ...
        Db::getInstance()->execute("CREATE TABLE IF NOT EXISTS `" . _DB_PREFIX_ . "pxpaypal_transactions` (
        ...
        Configuration::updateValue('PAYPAL_PX_ACCOUNT_NUMBER', '');
        Configuration::updateValue('PAYPAL_PX_ENCRYPTION_KEY', '');
        Configuration::updateValue('PAYPAL_PX_TESTMODE', 1);
        Configuration::updateValue('PAYPAL_PX_TRANSACTION_TYPE', 'AUTHORIZATION');
        Configuration::updateValue('PAYPAL_PX_PAYMENT_VIEW', 'PX');
        Configuration::updateValue('PAYPAL_PX_RESPONSIVE', false);
        return true;
    }
    ...
    public function uninstall()
    {
        ...
        //Db::getInstance()->execute("DROP TABLE IF EXISTS `" . _DB_PREFIX_ . "pxpaypal_transactions`; ");

        /* Clean configuration table */
        //Configuration::deleteByName('PAYPAL_PX_ACCOUNT_NUMBER');
        //Configuration::deleteByName('PAYPAL_PX_ENCRYPTION_KEY');
        //Configuration::deleteByName('PAYPAL_PX_TESTMODE');
        //Configuration::deleteByName('PAYPAL_PX_TRANSACTION_TYPE');
        //Configuration::deleteByName('PAYPAL_PX_PAYMENT_VIEW');
        //Configuration::deleteByName('PAYPAL_PX_RESPONSIVE');
    ...
    {
        if (Tools::isSubmit('btnSubmit')) {
            Configuration::updateValue('PAYPAL_PX_ACCOUNT_NUMBER', Tools::getValue('accountnumber'));
            Configuration::updateValue('PAYPAL_PX_ENCRYPTION_KEY', Tools::getValue('encryptionkey'));
            Configuration::updateValue('PAYPAL_PX_TESTMODE', Tools::getValue('mode'));
            Configuration::updateValue('PAYPAL_PX_TRANSACTION_TYPE', Tools::getValue('transactiontype'));
            Configuration::updateValue('PAYPAL_PX_PAYMENT_VIEW', Tools::getValue('paymentview'));
            Configuration::updateValue('PAYPAL_PX_RESPONSIVE', (bool)Tools::getValue('responsive'));
        }
    ...
    private function _displayForm()
    {
        ...
        $this->_html .= '<img src="../modules/pxpaypal/logo.gif" style="float:left; margin-right:15px;" width="86" height="49"><b>'
            ...
            . 'Use <a href="' . _PS_BASE_URL_ . __PS_BASE_URI__ . 'index.php?fc=module&module=pxpaypal&controller=transaction" target="_blank">this URL</a> as a Transaction Callback URL.' . '<br /><br /><br />';
        ...
        switch ((int)$result['transactionStatus']) {
            case 1:
                ...
                if ($result['pending'] === 'true') {
                    // Set Pending Status
                    $order->setCurrentState(Configuration::get('PS_OS_PAYEX_PENDING'));
                    $status = 'pending';
                } else {
                    // Cancel Unknown transaction status
                    $order->setCurrentState(Configuration::get('PS_OS_ERROR'));
                    $status = 'error';
                }
                break;
    ...
    public function hookBackOfficeHeader()
    {
        ...
        if (Tools::isSubmit('process_fetch') && Tools::getValue('payex_transaction_id')) {
            ...
            if (!empty($result['transactionStatus'])) {
                if (!Db::getInstance()->Execute('UPDATE `' . _DB_PREFIX_ . 'pxpaypal_transactions` SET transaction_status=' . pSQL((int)$result['transactionStatus']) . ', transaction_data="' . pSQL(serialize($result)) . '" WHERE transaction_id=' . $payex_transaction_id . ';')) {
    ...
    public function hookAdminOrder($params)
    {
        ...
        if ($order->module === $this->name) {
            ...
            $transactions = Db::getInstance()->executeS('SELECT * FROM ' . _DB_PREFIX_ . 'pxpaypal_transactions WHERE order_id = ' . $order_id . ' ORDER BY date DESC;');
    ...
    public function addTransaction($order_id, $transaction_id, $transaction_status, $transaction_data, $date = null)
    {
        ...
        // Save Transaction
        if (!Db::getInstance()->Execute(sprintf('INSERT INTO `' . _DB_PREFIX_ . 'pxpaypal_transactions` (order_id, transaction_id, transaction_status, transaction_data, date) VALUES (%s, %s, %s, "%s", "%s");',
    ...
    {
        $transaction = Db::getInstance()->executeS('SELECT * FROM ' . _DB_PREFIX_ . 'pxpaypal_transactions WHERE transaction_id = ' . (int) $transaction_id . ';');
    ...
    public function log($message)
    {
        if (!$this->_log) {
            ...
            $this->_log->setFilename(_PS_ROOT_DIR_ . '/log/pxpaypal_payment.log');
        }
