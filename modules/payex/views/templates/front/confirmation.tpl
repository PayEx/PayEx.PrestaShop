...
{if $status == 'ok'}
    <p>{l s='Your order on %s is complete.' sprintf=$shop_name mod='payex'}
        ...
        <br /><br />{l s='For any questions or for further information, please contact our' mod='payex'} <a href="{$link->getPageLink('contact', true)|escape:'htmlall':'UTF-8'}">{l s='customer support' mod='payex'}</a>.
    </p>
{else}
    {if $status == 'pending'}
    ...
    {else}
        <p class="warning">
            ...
            <a href="{$link->getPageLink('contact', true)|escape:'htmlall':'UTF-8'}">{l s='customer support' mod='payex'}</a>.
            <br />
