...
class Pxswish extends PaymentModule
{
    private $_html = '';
    ...
    public function __construct()
    {
        ...
        // Init Configuration
        $config = Configuration::getMultiple(array('SWISH_PX_ACCOUNT_NUMBER', 'SWISH_PX_ENCRYPTION_KEY', 'SWISH_PX_TESTMODE', 'SWISH_PX_RESPONSIVE', 'SWISH_PX_CHECKOUT_INFO'));
        $this->accountnumber = isset($config['SWISH_PX_ACCOUNT_NUMBER']) ? $config['SWISH_PX_ACCOUNT_NUMBER'] : '';
        $this->encryptionkey = isset($config['SWISH_PX_ENCRYPTION_KEY']) ? $config['SWISH_PX_ENCRYPTION_KEY'] : '';
        $this->mode = isset($config['SWISH_PX_TESTMODE']) ? $config['SWISH_PX_TESTMODE'] : 1;
        $this->responsive = isset($config['SWISH_PX_RESPONSIVE']) ? (bool)$config['SWISH_PX_RESPONSIVE'] : false;
        $this->checkout_info = isset($config['SWISH_PX_CHECKOUT_INFO']) ? (bool)$config['SWISH_PX_CHECKOUT_INFO'] : false;;
        ...
        $this->displayName = $this->l('Swish via PayEx');
        $this->description = $this->l('Accept payments for your products via Swish.');
    ...
    public function install()
    {
        ...
        Configuration::updateValue('SWISH_PX_ACCOUNT_NUMBER', '');
        Configuration::updateValue('SWISH_PX_ENCRYPTION_KEY', '');
        Configuration::updateValue('SWISH_PX_TESTMODE', 1);
        Configuration::updateValue('SWISH_PX_RESPONSIVE', false);
        Configuration::updateValue('SWISH_PX_CHECKOUT_INFO', true);
        return true;
    }
    ...
    public function uninstall()
    {
        ...
        // Please leave table "payex_transactions"
    ...
    {
        if (Tools::isSubmit('btnSubmit')) {
            Configuration::updateValue('SWISH_PX_ACCOUNT_NUMBER', Tools::getValue('accountnumber'));
            Configuration::updateValue('SWISH_PX_ENCRYPTION_KEY', Tools::getValue('encryptionkey'));
            Configuration::updateValue('SWISH_PX_TESTMODE', Tools::getValue('mode'));
            Configuration::updateValue('SWISH_PX_RESPONSIVE', (bool)Tools::getValue('responsive'));
            Configuration::updateValue('SWISH_PX_CHECKOUT_INFO', (bool)Tools::getValue('checkout_info'));
        }
    ...
    private function _displayForm()
    {
        ...
        $this->_html .= '<img src="../modules/pxswish/views/img/pxswish.png" style="float:left; margin-right:15px;" width="80" height="80"><b>'
            . $this->l('This module allows you to accept payments by Swish via PayEx.') . '</b><br /><br />';
    ...
    public function hookHeader($params)
    {
        ...
        if (file_exists( _PS_CACHE_DIR_) && is_writable( _PS_CACHE_DIR_)) {
            ...
            if (!$last_check || (time() > $last_check + (12 * 60 * 60))) {
                $last_check = time();
                Tools::file_get_contents($cache_directory . '/last_check', $last_check);
    ...
    public function log($message)
    {
        if (!$this->_log) {
            ...
            $this->_log->setFilename(_PS_ROOT_DIR_ . '/log/payex_swish.log');
        }
