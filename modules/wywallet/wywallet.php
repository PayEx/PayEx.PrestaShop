...
class Wywallet extends PaymentModule
{
    private $_html = '';
    ...
    public function __construct()
    {
        ...
        // Init Configuration
        $config = Configuration::getMultiple(array('PX_WYWALLET_ACCOUNT_NUMBER', 'PX_WYWALLET_ENCRYPTION_KEY', 'PX_WYWALLET_TESTMODE', 'PX_WYWALLET_TRANSACTION_TYPE', 'PX_WYWALLET_RESPONSIVE'));
        $this->accountnumber = isset($config['PX_WYWALLET_ACCOUNT_NUMBER']) ? $config['PX_WYWALLET_ACCOUNT_NUMBER'] : '';
        $this->encryptionkey = isset($config['PX_WYWALLET_ENCRYPTION_KEY']) ? $config['PX_WYWALLET_ENCRYPTION_KEY'] : '';
        $this->mode = isset($config['PX_WYWALLET_TESTMODE']) ? $config['PX_WYWALLET_TESTMODE'] : 1;
        $this->transactiontype = isset($config['PX_WYWALLET_TRANSACTION_TYPE']) ? $config['PX_WYWALLET_TRANSACTION_TYPE'] : 'AUTHORIZATION';
        $this->responsive = isset($config['PX_WYWALLET_RESPONSIVE']) ? (bool)$config['PX_WYWALLET_RESPONSIVE'] : false;
        ...
        $this->displayName = $this->l('WyWallet');
        $this->description = $this->l('Accept payments for your products via WyWallet.');
    ...
    public function install()
    {
        ...
        Db::getInstance()->execute("CREATE TABLE IF NOT EXISTS `" . _DB_PREFIX_ . "payex_wywallet_transactions` (
        ...
        Configuration::updateValue('PX_WYWALLET_ACCOUNT_NUMBER', '');
        Configuration::updateValue('PX_WYWALLET_ENCRYPTION_KEY', '');
        Configuration::updateValue('PX_WYWALLET_TESTMODE', 1);
        Configuration::updateValue('PX_WYWALLET_TRANSACTION_TYPE', 'AUTHORIZATION');
        Configuration::updateValue('PX_WYWALLET_RESPONSIVE', false);
        return true;
    }
    ...
    {
        if (Tools::isSubmit('btnSubmit')) {
            Configuration::updateValue('PX_WYWALLET_ACCOUNT_NUMBER', Tools::getValue('accountnumber'));
            Configuration::updateValue('PX_WYWALLET_ENCRYPTION_KEY', Tools::getValue('encryptionkey'));
            Configuration::updateValue('PX_WYWALLET_TESTMODE', Tools::getValue('mode'));
            Configuration::updateValue('PX_WYWALLET_TRANSACTION_TYPE', Tools::getValue('transactiontype'));
            Configuration::updateValue('PX_WYWALLET_RESPONSIVE', (bool)Tools::getValue('responsive'));
        }
    ...
    public function hookBackOfficeHeader()
    {
        ...
        if (Tools::isSubmit('process_fetch') && Tools::getValue('payex_transaction_id')) {
            ...
            if (!empty($result['transactionStatus'])) {
                if (!Db::getInstance()->Execute('UPDATE `' . _DB_PREFIX_ . 'payex_wywallet_transactions` SET transaction_status=' . pSQL((int)$result['transactionStatus']) . ', transaction_data="' . pSQL(serialize($result)) . '" WHERE transaction_id=' . $payex_transaction_id . ';')) {
    ...
    public function hookAdminOrder($params)
    {
        ...
        if ($order->module === $this->name) {
            ...
            $transactions = Db::getInstance()->executeS('SELECT * FROM ' . _DB_PREFIX_ . 'payex_wywallet_transactions WHERE order_id = ' . $order_id . ' ORDER BY date DESC;');
    ...
    public function addTransaction($order_id, $transaction_id, $transaction_status, $transaction_data, $date = null)
    {
        ...
        // Save Transaction
        if (!Db::getInstance()->Execute(sprintf('INSERT INTO `' . _DB_PREFIX_ . 'payex_wywallet_transactions` (order_id, transaction_id, transaction_status, transaction_data, date) VALUES (%s, %s, %s, "%s", "%s");',
    ...
    {
        $transaction = Db::getInstance()->executeS('SELECT * FROM ' . _DB_PREFIX_ . 'payex_wywallet_transactions WHERE transaction_id = ' . (int) $transaction_id . ';');
