...
class Pxoneclick extends PaymentModule
{
    private $_html = '';
    ...
    public function __construct()
    {
        ...
        // Init Configuration
        $config = Configuration::getMultiple(array('PX_OC_ACCOUNT_NUMBER', 'PX_OC_ENCRYPTION_KEY', 'PX_OC_TESTMODE', 'PX_OC_TRANSACTION_TYPE', 'PX_OC_AGREEMENT_URL', 'PX_OC_MAX_AMOUNT', 'PX_OC_RESPONSIVE'));
        $this->accountnumber = isset($config['PX_OC_ACCOUNT_NUMBER']) ? $config['PX_OC_ACCOUNT_NUMBER'] : '';
        $this->encryptionkey = isset($config['PX_OC_ENCRYPTION_KEY']) ? $config['PX_OC_ENCRYPTION_KEY'] : '';
        $this->mode = isset($config['PX_OC_TESTMODE']) ? $config['PX_OC_TESTMODE'] : 1;
        $this->transactiontype = isset($config['PX_OC_TRANSACTION_TYPE']) ? $config['PX_OC_TRANSACTION_TYPE'] : 'AUTHORIZATION';
        $this->agreement_url = isset($config['PX_OC_AGREEMENT_URL']) ? $config['PX_OC_AGREEMENT_URL'] : '';
        $this->max_amount = isset($config['PX_OC_MAX_AMOUNT']) ? (float)$config['PX_OC_MAX_AMOUNT'] : 0;
        $this->responsive = isset($config['PX_OC_RESPONSIVE']) ? (bool)$config['PX_OC_RESPONSIVE'] : false;
        ...
        $this->displayName = $this->l('Payex One Click');
    ...
    public function install()
    {
        ...
        Db::getInstance()->execute("CREATE TABLE IF NOT EXISTS `" . _DB_PREFIX_ . "payex_autopay_transactions` (
        ...
        // Create table for Agreement References
        Db::getInstance()->execute("CREATE TABLE IF NOT EXISTS `" . _DB_PREFIX_ . "payex_autopay` (
            `customer_id` int(10) NOT NULL COMMENT 'Customer Id',
            `agreement_ref` varchar(255) NOT NULL COMMENT 'Agreement Reference',
            `max_amount` float NOT NULL COMMENT 'Max Transaction Amount',
            UNIQUE KEY `customer_id` (`customer_id`)
            ) ENGINE=" . _MYSQL_ENGINE_ . " DEFAULT CHARSET=utf8;
        ");

        // Set Payment Settings
        Configuration::updateValue('PX_OC_ACCOUNT_NUMBER', '');
        Configuration::updateValue('PX_OC_ENCRYPTION_KEY', '');
        Configuration::updateValue('PX_OC_TESTMODE', 1);
        Configuration::updateValue('PX_OC_TRANSACTION_TYPE', 'AUTHORIZATION');
        Configuration::updateValue('PX_OC_AGREEMENT_URL', _PS_BASE_URL_ . __PS_BASE_URI__);
        Configuration::updateValue('PX_OC_MAX_AMOUNT', 0);
        Configuration::updateValue('PX_OC_RESPONSIVE', false);
        return true;
    }
    ...
    private function _postValidation()
    {
        if (Tools::isSubmit('btnSubmit')) {
            ...
            if (!Tools::getValue('agreement_url')) {
                $this->_postErrors[] = $this->l('Agreement URL is required.');
            } elseif (!Validate::isUrl(Tools::getValue('agreement_url'))) {
                $this->_postErrors[] = $this->l('Agreement URL is invalid URL.');
            }

            if (!Tools::getValue('max_amount')) {
                $this->_postErrors[] = $this->l('Max amount is required.');
            } elseif (!is_numeric(Tools::getValue('max_amount'))) {
                $this->_postErrors[] = $this->l('Max amount is invalid.');
            }
        }
    }
    ...
    {
        if (Tools::isSubmit('btnSubmit')) {
            Configuration::updateValue('PX_OC_ACCOUNT_NUMBER', Tools::getValue('accountnumber'));
            Configuration::updateValue('PX_OC_ENCRYPTION_KEY', Tools::getValue('encryptionkey'));
            Configuration::updateValue('PX_OC_TESTMODE', Tools::getValue('mode'));
            Configuration::updateValue('PX_OC_TRANSACTION_TYPE', Tools::getValue('transactiontype'));
            Configuration::updateValue('PX_OC_AGREEMENT_URL', Tools::getValue('agreement_url'));
            Configuration::updateValue('PX_OC_MAX_AMOUNT', Tools::getValue('max_amount'));
            Configuration::updateValue('PX_OC_RESPONSIVE', (bool)Tools::getValue('responsive'));
        }
    ...
    private function _displayForm()
    {
        ...
        $this->_html .= '<img src="../modules/pxoneclick/logo.gif" style="float:left; margin-right:15px;" width="86" height="49"><b>'
            ...
            . 'Use <a href="' . _PS_BASE_URL_ . __PS_BASE_URI__ . 'index.php?fc=module&module=pxoneclick&controller=transaction" target="_blank">this URL</a> as a Transaction Callback URL.' . '<br /><br /><br />';
        ...
        $this->_html .=
			...
			<legend><img src="../img/admin/contact.gif" />' . $this->l('Settings') . '</legend>
				<table border="0" width="500" cellpadding="0" cellspacing="0" id="form">
					...
					<tr>
						<td width="130" style="vertical-align: top;">' . $this->l('Agreement url') . '</td>
						<td><input type="text" name="agreement_url" value="' . htmlentities(Tools::getValue('agreement_url', $this->agreement_url), ENT_COMPAT, 'UTF-8') . '" style="width: 300px;" /></td>
					</tr>
					<tr>
						<td width="130" style="vertical-align: top;">' . $this->l('Max amount of single transaction') . '</td>
						<td><input type="text" name="max_amount" value="' . htmlentities(Tools::getValue('max_amount', $this->max_amount), ENT_COMPAT, 'UTF-8') . '" style="width: 300px;" /></td>
					</tr>
					<tr>
    ...
    public function hookPaymentReturn($params)
    {
        ...
        $autopay_token = Tools::getValue('autopay_token');

        if (!empty($transaction_id) && !empty($autopay_token)) {
            // When Direct AutoPay
            if ($autopay_token !== md5(_COOKIE_KEY_ . 'transactionId_' . $transaction_id)) {
                die('Wrong autopay token!');
            }

            // Extract Transaction Data
            $transactions = Db::getInstance()->executeS('SELECT * FROM ' . _DB_PREFIX_ . 'payex_autopay_transactions WHERE transaction_id = ' . (int)$transaction_id . ';');
            ...
            $result = array(
                ...
                'transactionNumber' => $transactions['transaction_id'],
                'date' => $transactions['date']
            );
        } elseif (!empty($orderRef)) {
            ...
            die('Error: No parameters are specified.');
        }
    ...
    public function hookBackOfficeHeader()
    {
        ...
        if (Tools::isSubmit('process_fetch') && Tools::getValue('payex_transaction_id')) {
            ...
            if (!empty($result['transactionStatus'])) {
                if (!Db::getInstance()->Execute('UPDATE `' . _DB_PREFIX_ . 'payex_autopay_transactions` SET transaction_status=' . pSQL((int)$result['transactionStatus']) . ', transaction_data="' . pSQL(serialize($result)) . '" WHERE transaction_id=' . $payex_transaction_id . ';')) {
    ...
    public function hookAdminOrder($params)
    {
        ...
        if ($order->module === $this->name) {
            ...
            $transactions = Db::getInstance()->executeS('SELECT * FROM ' . _DB_PREFIX_ . 'payex_autopay_transactions WHERE order_id = ' . $order_id . ' ORDER BY date DESC;');
    ...
    public function addTransaction($order_id, $transaction_id, $transaction_status, $transaction_data, $date = null)
    {
        ...
        // Save Transaction
        if (!Db::getInstance()->Execute(sprintf('INSERT INTO `' . _DB_PREFIX_ . 'payex_autopay_transactions` (order_id, transaction_id, transaction_status, transaction_data, date) VALUES (%s, %s, %s, "%s", "%s");',
    ...
    {
        $transaction = Db::getInstance()->executeS('SELECT * FROM ' . _DB_PREFIX_ . 'payex_autopay_transactions WHERE transaction_id = ' . (int) $transaction_id . ';');
    ...
    public function log($message)
    {
        if (!$this->_log) {
            ...
            $this->_log->setFilename(_PS_ROOT_DIR_ . '/log/pxonclick_payment.log');
        }
